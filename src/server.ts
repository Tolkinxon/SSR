...
const server = http.createServer(async(req, res)=>{
    ...
    else if(reqUrl.startsWith('/api/auth/login') && reqMethod == METHODS.CREATE) authController.login(req, res); 
    ...
    else if(reqUrl.includes('/css') || reqUrl.includes('/js') || reqUrl.includes('/img')) JsAndCssLoadController.jsAndCssLoad(req, res); 
